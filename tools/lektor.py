...
def pause_after(text, min_pause, smart=True):
    """Zwraca faktycznie wymaganą pauzę po kwestii."""
...
def atempo_filter_chain(factor):
    """Buduje filtr atempo dla współczynnika > 1.0."""
...
def synthesize_and_fit(entries, tts, style, total_steps, base_speed, max_speed, ffmpeg, tmp,
                       min_pause=DEFAULT_MIN_PAUSE, smart_pauses=True):
    """
    Dopasowanie lektora z twardym limitem max_speed.

    Ważne zasady v10:
      * max_speed jest absolutnym limitem przyspieszenia; żadna ścieżka awaryjna
        nie może go ominąć.
      * pauza jest realną ciszą pomiędzy kolejnymi kwestiami, o ile timing SRT
        faktycznie na nią pozwala.
      * jeżeli kwestia nie mieści się nawet przy max_speed, nie przyspieszamy jej
        dalej. W logu pojawia się ostrzeżenie i zachowujemy najwolniejsze
        dopuszczalne przez ustawienia tempo.
      * start kwestii jest zakotwiczony w czasie SRT, zamiast przesuwać kolejny
        dialog o stałe min_pause.
    """
    ...
    # Obrona przed wartościami spoza GUI/CLI.
    base_speed = max(0.10, float(base_speed))
    max_speed = max(1.00, min(float(max_speed), NATIVE_SPEED_CAP))
    min_pause = max(0.08, min(0.50, float(min_pause)))
    ...
    for i, (start, end, text) in enumerate(entries):
        ...
        # Normalnie trzymamy początek dokładnie według SRT. Tylko gdy poprzednia
        # kwestia wyszła poza ten punkt (bo była zbyt długa przy twardym limicie),
        # odsuwamy bieżącą, aby audio nie nakładało się na siebie.
        actual = max(float(start), cur_end)

        desired_pause = pause_after(text, min_pause, smart_pauses)
        next_start = entries[i + 1][0] if i + 1 < n else None

        if next_start is not None:
            # Docelowy slot kończy się przed następnym dialogiem, pozostawiając
            # rzeczywistą pauzę. Jeśli bieżąca kwestia zaczęła się później z powodu
            # poprzedniej, dostępne miejsce jest odpowiednio mniejsze.
            slot = max(0.05, float(next_start) - actual - desired_pause)
        else:
            slot = dur

        use = src
        if dur > slot + 0.02 and next_start is not None:
            needed_tempo = dur / slot

            # Najpierw ponowna synteza z wyższym tempem. Nigdy ponad max_speed.
            if needed_tempo >= RESYNTH_TRIGGER:
                native_speed = min(needed_tempo, max_speed) * base_speed
                # base_speed jest tempem bazowym Supertonic; nie pozwalamy,
                # aby iloczyn wyszedł ponad absolutny limit użytkownika.
                native_speed = min(native_speed, max_speed)
                if native_speed > base_speed + 0.01:
                    resynth = os.path.join(clips, "%06d_r.wav" % i)
                    try:
                        synth_one(tts, style, text, total_steps, native_speed, resynth, rate)
                        if os.path.isfile(resynth):
                            use = resynth
                            dur = clip_dur(resynth)
                    except Exception as ex:
                        sys.stderr.write("\nOstrzezenie przy ponownej syntezie linii %d: %s\n" % (i, ex))

            # Jeżeli nadal za długo, używamy atempo, ale twardo ograniczonego do
            # max_speed. Nie ma już żadnego emergency_tempo omijającego ustawienie.
            if dur > slot + 0.02:
                tempo = min(dur / slot, max_speed)
                if tempo > 1.01:
                    dst = os.path.join(clips, "%06d_f.wav" % i)
                    r = run([ffmpeg, "-y", "-v", "error", "-i", use,
                             "-filter:a", atempo_filter_chain(tempo),
            ...
            # Jeżeli nadal się nie mieści, NIE przekraczamy max_speed. To jest
            # celowe: użytkownik ustawił twardy limit. Pauza może w takim przypadku
            # zostać skrócona lub zniknąć, ale tempo nie zostanie przekroczone.
            if dur > slot + 0.02:
                required = dur / max(0.05, slot)
                sys.stderr.write(
                    "\nUWAGA: kwestia %d nie miesci sie przy maksymalnym tempie %.2fx "
                    "(wymagane ok. %.2fx). Nie przekraczam ustawionego limitu.\n"
                    % (i + 1, max_speed, required)
                )
